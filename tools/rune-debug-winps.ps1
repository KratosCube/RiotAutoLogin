...
function Shorten-Body {
    ...
    if ($compact.Length -le $Length) { return $compact }
...
function Invoke-LcuGet {
    ...
    try {
        $response = Invoke-WebRequest -Uri "https://127.0.0.1:$Port/$Endpoint" -Headers $Headers -Method GET -UseBasicParsing -TimeoutSec 5
        return [ordered]@{
            endpoint = $Endpoint
...
if (-not $client) { throw "LeagueClientUx.exe is not running. Open League Client and enter champion select first." }

$commandLine = [string]$client.CommandLine
$portMatch = [regex]::Match($commandLine, '--app-port="?(\d+)"?')
$authMatch = [regex]::Match($commandLine, '--remoting-auth-token=([a-zA-Z0-9_-]+)')
if (-not $portMatch.Success -or -not $authMatch.Success) { throw "Could not read League Client LCU auth from LeagueClientUx command line." }

$Port = $portMatch.Groups[1].Value
$auth = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes("riot:" + $authMatch.Groups[1].Value))
...
$results = foreach ($endpoint in ($endpoints | Select-Object -Unique)) { Invoke-LcuGet $endpoint }
