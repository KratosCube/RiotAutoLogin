...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    .rune-button { display:grid; grid-template-columns:minmax(0,1fr) auto; align-items:start; gap:8px; padding:8px; text-align:left; text-transform:none; letter-spacing:0; min-height:40px; }
    .rune-button.current { border-color:rgba(10,200,185,.55); color:#d7fffb; }
    .rune-button.recommended { border-color:rgba(240,213,138,.45); }
    .rune-button span { min-width:0; overflow:visible; text-overflow:clip; white-space:normal; line-height:1.25; }
    .rune-button small { color:var(--muted); display:block; margin-top:3px; font:10px system-ui,sans-serif; white-space:normal; line-height:1.35; }
    .rune-button em { color:var(--gold); font-style:normal; font-size:10px; text-transform:uppercase; letter-spacing:.08em; padding-top:2px; }
...
const championsEl=document.getElementById('champions'), allyPickedListEl=document.getElementById('allyPickedList'), enemyPickedListEl=document.getElementById('enemyPickedList'), phaseEl=document.getElementById('phase'), messageEl=document.getElementById('message'), turnEl=document.getElementById('turn'), gameStatusEl=document.getElementById('gameStatus'), timerEl=document.getElementById('timer'), timerLabelEl=document.getElementById('timerLabel'), acceptedAlertEl=document.getElementById('acceptedAlert'), searchEl=document.getElementById('search'), countEl=document.getElementById('count'), toastEl=document.getElementById('toast'), desktopLoadoutEl=document.getElementById('desktopLoadout'), mobileLoadoutEl=document.getElementById('mobileLoadout'), spellModalEl=document.getElementById('spellModal'), spellGridEl=document.getElementById('spellGrid'), spellModalTitleEl=document.getElementById('spellModalTitle'), leaveButtonEl=document.getElementById('leaveButton');
...
function syncTimer(s){const nextKey=timerIdentity(s);const keyChanged=nextKey!==timerKey;const hasValid=typeof s.timeLeftInPhaseMs==='number'&&s.timeLeftInPhaseMs>=0;if(!hasValid){const previousMs=currentTimerMs();const keepPrevious=Boolean(s.isInChampSelect)&&timerBaseMs!==null&&previousMs!==null&&previousMs!==Infinity;if(keepPrevious)return;timerInfinite=Boolean(s.isTimerInfinite);timerBaseMs=null;timerBasePerf=performance.now();timerKey=nextKey;lastVisibleSecond=null;return;}timerInfinite=Boolean(s.isTimerInfinite);const serverMs=Math.max(0,s.timeLeftInPhaseMs);const currentMs=currentTimerMs();if(keyChanged||currentMs===null||currentMs===Infinity){timerBaseMs=serverMs;timerBasePerf=performance.now();timerKey=nextKey;lastVisibleSecond=null;return;}const serverIsLower=serverMs<currentMs-150;const hardResyncAfterZero=currentMs<500&&serverMs>2000;if(serverIsLower||hardResyncAfterZero){timerBaseMs=serverMs;timerBasePerf=performance.now();if(hardResyncAfterZero)lastVisibleSecond=null;} }
...
function renderRecommendedRunes(){const pages=state.recommendedRunePages||[];if(!pages.length)return `<div class="rune-block"><div class="rune-current">Recommended runes unavailable</div></div>`;return `<div class="rune-block"><div class="rune-current">Recommended by Riot</div><div class="rune-list">${pages.map(p=>`<button class="rune-button recommended" data-recommended-rune-index="${p.index}" ${p.canApply===false?'disabled':''}><span><strong>${escapeHtml(p.name||('Recommended '+(p.index+1)))}</strong><small>${escapeHtml(p.subtitle||`${p.selectedPerkIds?.length||0} perks`)}</small></span><em>Apply</em></button>`).join('')}</div></div>`;}
...
function hoverChampion(id,name){showToast(`${name} intent sent.`);fetch('/api/hover',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({championId:id})}).then(r=>r.json()).then(result=>{if(!result.success)showToast(result.message||'Hover failed.');setTimeout(loadTimer,120);setTimeout(loadState,600);}).catch(()=>showToast('Hover failed.'));}
async function pickChampion(id,name){if(!confirm(`Lock in ${name}?`))return;await runChampionAction('/api/pick',id,'Champion locked in.');} async function banChampion(id,name){if(!confirm(`Ban ${name}?`))return;await runChampionAction('/api/ban',id,'Champion banned.');}
...
loadState();loadTimer();setInterval(loadTimer,750);setInterval(loadState,3000);setInterval(updateTimerDisplay,100);
</script>
</body>
</html>
""";
