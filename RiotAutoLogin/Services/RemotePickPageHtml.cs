...
function syncTimer(s){const nextKey=timerIdentity(s);const keyChanged=nextKey!==timerKey;timerInfinite=Boolean(s.isTimerInfinite);if(typeof s.timeLeftInPhaseMs!=='number'||s.timeLeftInPhaseMs<0){const keepPrevious=Boolean(s.isInChampSelect)&&timerBaseMs!==null&&currentTimerMs()!==null;if(keepPrevious){timerKey=nextKey||timerKey;return;}timerBaseMs=null;timerBasePerf=performance.now();timerKey=nextKey;lastVisibleSecond=null;return;}const serverMs=Math.max(0,s.timeLeftInPhaseMs);const currentMs=currentTimerMs();if(keyChanged||currentMs===null||currentMs===Infinity){timerBaseMs=serverMs;timerBasePerf=performance.now();timerKey=nextKey;lastVisibleSecond=null;return;}const serverIsLower=serverMs<currentMs-150;const hardResyncAfterZero=currentMs<500&&serverMs>2000;if(serverIsLower||hardResyncAfterZero){timerBaseMs=serverMs;timerBasePerf=performance.now();if(hardResyncAfterZero)lastVisibleSecond=null;} }
