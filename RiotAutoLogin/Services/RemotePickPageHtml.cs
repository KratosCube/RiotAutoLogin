...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
    ...
    @media (max-width:900px) { .app{padding:8px;padding-bottom:96px}.sticky-status{margin:-8px -8px 8px;padding:8px}.topbar{grid-template-columns:1fr;margin-bottom:7px}.side-title{display:none}h1{font-size:20px}.phase-main{padding:9px}.message{font-size:13px;white-space:normal;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical}.timer-box{min-width:76px;padding:7px}.timer-value{font-size:25px}.layout{grid-template-columns:1fr;gap:8px}.panel.side{display:none}.mobile-loadout{display:block;margin-bottom:9px}.search-wrap{grid-template-columns:1fr 54px;position:sticky;top:151px;z-index:8;background:rgba(3,7,13,.92);padding-bottom:8px}.champion-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}.champion{min-height:162px}.portrait{height:82px}.champion-name{font-size:10px;padding:7px 6px}.champion-actions{gap:5px;padding:0 5px 6px}.champion-actions button{min-height:32px;font-size:9px;letter-spacing:.04em}.spell-grid{grid-template-columns:1fr 1fr} }
  </style>
</head>
<body>
  <div class="app">
...
const championsEl=document.getElementById('champions'), pickedListEl=document.getElementById('pickedList'), bannedListEl=document.getElementById('bannedList'), phaseEl=document.getElementById('phase'), messageEl=document.getElementById('message'), turnEl=document.getElementById('turn'), timerEl=document.getElementById('timer'), acceptedAlertEl=document.getElementById('acceptedAlert'), searchEl=document.getElementById('search'), countEl=document.getElementById('count'), toastEl=document.getElementById('toast'), desktopLoadoutEl=document.getElementById('desktopLoadout'), mobileLoadoutEl=document.getElementById('mobileLoadout'), spellModalEl=document.getElementById('spellModal'), spellGridEl=document.getElementById('spellGrid'), spellModalTitleEl=document.getElementById('spellModalTitle'), leaveButtonEl=document.getElementById('leaveButton');
...
function updateTimerDisplay(){const ms=currentTimerMs();if(ms===Infinity){timerEl.textContent='∞';lastVisibleSecond=null;return;}if(ms===null){timerEl.textContent='--';lastVisibleSecond=null;return;}const raw=Math.max(0,Math.ceil(ms/1000));const visible=lastVisibleSecond===null?raw:Math.min(lastVisibleSecond,raw);lastVisibleSecond=visible;timerEl.textContent=String(visible);}
